...
col1.metric(label="`wpm`: *words per minute*", value=typing['wpm'].median(), help='**Median** words per minute')
col2.metric(label="`rawWpm`: *raw words per minute*", value=typing['rawWpm'].median(), help='**Median** raw words per minute, a metric of wpm and accuracy')
col3.metric(label="`acc`, `consistency`: *accuracy and consistency*", value=f"{round(typing['acc'].median(), 1)}%,  {round(typing['consistency'].median(),1)}%", help='**Median** accuracy and consistency, represented as percentages.')
